...
fillPeakSummary <- function() {
  for (i in 1:totalRows) { #nrow(betas)) {
    ...
    # 5c: for potentially bimodal distributions, flag probe if 
    # one peak is very large (e.g. has >= 95% of samples) AND
    # the minima in between is close to zero within a certain tolerance
    # (e.g. < 1/sample size, 1e-6)
    tailDriven <- FALSE
    if (nrow(detected) == 2) {
      if ((sum(detected$propSample > 0.95) > 0) && 
          (probeDensityEst$y[detected$rightMinIdx[1]] < pushToZero)) {
        tailDriven <- TRUE
      }
    }
