...
peakSummaryPlot <- function(probe.names.list = NULL, row.id = NULL, probe.id = "", 
                              ...
                              histogram.bins = 500, histogram.col = "black",
  ...
  if (noProbeName) { # row.id must have been provided
  ...
  fittedDensity <- density(betas[betaIndex,], from = 0, to = 1, n = numBreaks, 
  ...
  hist(betas[betaIndex,], breaks = histogram.bins, xlim = c(0,1), 
...
seePeaks <- function(row.id = NULL, probe.id = "", histogram.bins = 500, 
  ...
  # DEBUG: Check plot
  seeDetected(original.data = betas[row.id,], fitted.density = fittedDensity, 
              detected.peaks = detectedPeaks, row.id = row.id)
  # browser()
  ...
  if (length(detectedPeaks$maximaIdx) > 1) {
    ...
    # # DEBUG: Check plot
    # seeDetected(original.data = betas[row.id,], fitted.density = fittedDensity, detected.peaks = detectedPeaks, row.id = row.id)
    # browser()
  ...
  text(fittedDensity$x[detectedPeaks$maximaIdx], 
       fittedDensity$y[detectedPeaks$maximaIdx], col = label.col,
