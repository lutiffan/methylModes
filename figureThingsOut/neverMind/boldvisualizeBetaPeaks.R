...
peakSummaryPlot <- function(probe.names.list = NULL, row.id = NULL, probe.id = "", 
  ...
  peakLabels <- as.character(round(detectedPeaks, 2))
  # peakLabels <- c('a', 'b')
  peakLabels <- paste0("paste(bold(", peakLabels, "))")
  ...
  for (l in seq_along(peakLabels)) {
    text(detectedPeaks[l], fittedPeaks[l], col = label.col,
         label = parse(text = peakLabels[l]), pos = 3)
  }
