...
fillPeakSummaryParallel <- function(betas = NULL) {
  ...
  # # Use progressr::withProgressShiny for Shiny progress bar
  ...
  #                                                 foundPeaks <- methylModes(row.data = as.vector(probe))
  ...
  betaIterator <- iter(betas, by = "row")
