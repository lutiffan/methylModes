...
# 21798
# 25250
# 25838 bimodal instead of tri due to threshold
# 30490
# 33367
# unimodal blob: cg08578320
# minima not too close to zero: 37603
# 40681 !!
# sparse long tail: 43355, 43973, cg13101431
...
gainedCtoTPD <- intersect(teenMultiModalProbes, childMultiModalProbes)

length(sameCtoTPD)/(length(unique(c(childMultiModalProbes, teenMultiModalProbes))))
```
Spot check differences
```{r}
...
```
Try a post-processing step
```{r}
noisyTailChild <- logical(nrow(peakSummaryChild))
for (i in 1:nrow(noisyTailChild)) {
  # TODO: include peak height in peakSummary
  if (peakSummaryChild[i,]$numPeaks != 2) {
    next
  }
  peak1Ht <- peakSummaryChild[i,]$fittedPeakHeight[1]
  peak2Ht <- peakSummaryChild[i,]$fittedPeakHeight[2]
  noisyTailChild[i] <- min(peak1Ht, peak2Ht) < 0.1 * max(peak1Ht, peak2Ht)
}
```
